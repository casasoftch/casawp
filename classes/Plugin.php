...
class Plugin
{
    ...
    public function properties_shortcode($args = array())
    {
        ...
        switch ($viewgroup) {
            case 'bootstrap2':
                $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap2/casawp-shortcode-properties.php';
                break;
            case 'bootstrap4':
                $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap4/casawp-shortcode-properties.php';
                break;
            default:
                $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp-shortcode-properties.php';
                break;
        }
    ...
    public function getCategories()
    {
        ...
        if ($availabilities) {
            ...
            $category_slug_array = array_map(function ($item) {
                return $item['slug'];
            }, $category_property_count);
    ...
    public function getRegions()
    {
        ...
        if ($availability) {
            ...
            $region_slug_array = array_map(function ($item) {
                return $item['slug'];
    ...
    public function getUtilities()
    {
        ...
        //availabilites reduces utilities
        $availabilities = $this->getQueriedArrayAvailability();
        if (!empty($availabilities)) {
            global $wpdb;

            // Generate placeholders for each availability term to safely inject them into the query
            $placeholders = implode(',', array_fill(0, count($availabilities), '%s'));

            // Prepare SQL query using multiple availability terms
            $query = $wpdb->prepare(
                "SELECT {$wpdb->prefix}terms.term_id, {$wpdb->prefix}terms.slug
                FROM {$wpdb->prefix}terms
                INNER JOIN {$wpdb->prefix}term_taxonomy ON {$wpdb->prefix}term_taxonomy.term_id = {$wpdb->prefix}terms.term_id
                    AND {$wpdb->prefix}term_taxonomy.taxonomy = 'casawp_utility'
                INNER JOIN {$wpdb->prefix}term_relationships ON {$wpdb->prefix}term_relationships.term_taxonomy_id = {$wpdb->prefix}term_taxonomy.term_taxonomy_id
                INNER JOIN {$wpdb->prefix}posts ON {$wpdb->prefix}term_relationships.object_id = {$wpdb->prefix}posts.ID
                    AND {$wpdb->prefix}posts.post_status = 'publish'
                INNER JOIN {$wpdb->prefix}term_relationships AS referenceCheck ON referenceCheck.object_id = {$wpdb->prefix}posts.ID
                INNER JOIN {$wpdb->prefix}term_taxonomy AS referenceCheckTermTax ON referenceCheck.term_taxonomy_id = referenceCheckTermTax.term_taxonomy_id
                    AND referenceCheckTermTax.taxonomy = 'casawp_availability'
                INNER JOIN {$wpdb->prefix}terms AS referenceCheckTerms ON referenceCheckTerms.term_id = referenceCheckTermTax.term_id
                    AND referenceCheckTerms.slug IN ($placeholders)
                GROUP BY {$wpdb->prefix}terms.term_id",
                ...$availabilities // Spread the availabilities array into placeholders
            );

            $utility_property_count = $wpdb->get_results($query, ARRAY_A);

            $utility_slug_array = array_map(function ($item) {
                return $item['slug'];
            }, $utility_property_count);
        ...
        // Get queried sales type and reduce utilities further if applicable
        $salestype = $this->getQueriedSingularSalestype();
        ...
        if ($salestype) {
            ...
            $query = $wpdb->prepare(
                ...
                INNER JOIN {$wpdb->prefix}term_relationships AS referenceCheck ON referenceCheck.object_id = {$wpdb->prefix}posts.ID
                INNER JOIN {$wpdb->prefix}term_taxonomy AS referenceCheckTermTax ON referenceCheck.term_taxonomy_id = referenceCheckTermTax.term_taxonomy_id
                    AND referenceCheckTermTax.taxonomy = 'casawp_salestype'
                INNER JOIN {$wpdb->prefix}terms AS referenceCheckTerms ON referenceCheckTerms.term_id = referenceCheckTermTax.term_id
                    AND referenceCheckTerms.slug = %s",
                $salestype
            );

            if (!empty($availabilities)) {
                $query .= $wpdb->prepare(
                    " INNER JOIN {$wpdb->prefix}term_relationships AS referenceCheckAvailability ON referenceCheckAvailability.object_id = {$wpdb->prefix}posts.ID
                    INNER JOIN {$wpdb->prefix}term_taxonomy AS referenceCheckAvailabilityTermTax ON referenceCheckAvailability.term_taxonomy_id = referenceCheckAvailabilityTermTax.term_taxonomy_id
                        AND referenceCheckAvailabilityTermTax.taxonomy = 'casawp_availability'
                    INNER JOIN {$wpdb->prefix}terms AS referenceCheckAvailabilityTerms ON referenceCheckAvailabilityTerms.term_id = referenceCheckAvailabilityTermTax.term_id
                        AND referenceCheckAvailabilityTerms.slug IN ($placeholders)",
                    ...$availabilities
                );
            }
    ...
    public function getSalestypes()
    {
        ...
        foreach ($salestype_terms as $salestype_term) {
            switch ($salestype_term->slug) {
                case 'rent':
                    $salestypes[$salestype_term->slug] = __('Rent', 'casawp');
                    break;
                case 'buy':
                    $salestypes[$salestype_term->slug] = __('Buy', 'casawp');
                    break;
                default:
                    $salestypes[$salestype_term->slug] = $salestype_term->slug;
                    break;
            }
        }
        return $salestypes;
    ...
    public function getAvailabilities()
    {
        ...
        foreach ($availability_terms as $availability_term) {
            switch ($availability_term->slug) {
                case 'active':
                    $availabilities[$availability_term->slug] = __('Active', 'casawp');
                    break;
                case 'reference':
                    $availabilities[$availability_term->slug] = __('Reference', 'casawp');
                    break;
                default:
                    $availabilities[$availability_term->slug] = $availability_term->slug;
                    break;
            }
        }
    ...
    public function getLocations()
    {
        ...
        if ($availabilities) {
            ...
            $location_id_array = array_map(function ($item) {
                return $item['term_id'];
            }, $location_property_count);
    ...
    public function getFeatures()
    {
        ...
        if ($availability) {
          ...
          GROUP BY " . $wpdb->prefix . "terms.term_id";
            ...
            $feature_id_array = array_map(function ($item) {
                return $item['term_id'];
            }, $feature_property_count);
    ...
    public function include_template_function($template_path)
    {
        ...
        if (get_post_type() == 'casawp_property' && is_single()) {
            if ($_GET && (isset($_GET['ajax']) || isset($_GET['json'])  || isset($_GET['casawp_map']))) {
                ...
                switch ($viewgroup) {
                    case 'bootstrap2':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap2/casawp-single.php';
                        break;
                    case 'bootstrap4':
        ...
        if (is_post_type_archive('casawp_property') || is_tax('casawp_salestype') || is_tax('casawp_availability') || is_tax('casawp_category') || is_tax('casawp_location') || is_tax('casawp_feature')) {
            if ($_GET && (isset($_GET['casawp_map']) || isset($_GET['ajax']) || isset($_GET['json']))) {
                ...
                switch ($viewgroup) {
                    case 'bootstrap2':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap2/casawp-archive.php';
                        break;
                    case 'bootstrap4':
        ...
        if (get_post_type() == 'casawp_project' && is_single()) {
            if ($_GET && (isset($_GET['ajax']) || isset($_GET['json']))) {
                ...
                switch ($viewgroup) {
                    case 'bootstrap2':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap2/casawp-project-single.php';
                        break;
                    case 'bootstrap4':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap4/casawp-project-single.php';
                        break;
                    default:
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp-project-single.php';
                        break;
                }
        ...
        if (is_post_type_archive('casawp_project')) {
            if ($_GET && (isset($_GET['ajax']) || isset($_GET['json']))) {
                ...
                switch ($viewgroup) {
                    case 'bootstrap2':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap2/casawp-project-archive.php';
                        break;
                    case 'bootstrap4':
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp/bootstrap4/casawp-project-archive.php';
                        break;
                    default:
                        $template_path = CASASYNC_PLUGIN_DIR . 'theme-defaults/casawp-project-archive.php';
                        break;
                }
