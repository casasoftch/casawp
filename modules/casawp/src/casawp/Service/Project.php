...
class Project{
    ...
    public function getIntegratedProjects(){
    	$projects = $this->getFieldValue('integratedprojects', false);
    	...
    	foreach ($projects as $project) {
    		$found = false;
    		foreach ($f_projects as $f_key => $f_project) {
    			$check_a = array_intersect_key($f_project, array_flip($check_keys));
    			$check_b = array_intersect_key($project, array_flip($check_keys));
    			asort($check_a);
    			asort($check_b);
    			if ($check_a == $check_b) {
    				$found = $f_key;
    		...
    		if ($found) {
    			$f_projects[$found]['count'] = (isset($f_projects[$found]['count']) ? $f_projects[$found]['count'] : 1) + 1;
    		} else {
    			$project['count'] = 1;
    			$f_projects[] = $project;
    		}
    	...
    	$r_projects = array();
    	foreach ($f_projects as $project) {
    		if ($this->integratedProjectService->keyExists($project["type"])) {
    			$r_project = $this->integratedProjectService->getItem($project["type"]);
    			$r_project->setCost($project["price"]);
    			$r_project->setTimesegment($project["timesegment"]);
    			$r_project->setPropertysegment($project["propertysegment"]);
    			$r_project->setInclusive($project["inclusive"]);
    			$r_project->setCount($project["count"]);
    			$r_projects[] = $r_project;
    		} else {
    			$unknown_project = new \CasasoftStandards\Service\IntegratedProject();
    			$unknown_project->setKey($project["type"]);
    			$unknown_project->setLabel('?'.$project["type"]);
    			$r_projects[] = $unknown_project;
    		}
    	}
    	return $r_projects;
    }
	...
	public function renderDatatableProject(){
		return $this->render('datatable-project', array(
			'project' => $this
		));
	}
    ...
    public function renderIntegratedProjects(){
    	return $this->render('integrated-projects', array(
    		'project' => $this,
